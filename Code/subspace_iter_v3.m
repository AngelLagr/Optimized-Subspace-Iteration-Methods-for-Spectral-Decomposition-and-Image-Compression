...
function [ V, D, n_ev, it, itv, flag ] = subspace_iter_v3( A, m, percentage, p, eps, maxit )
    ...
    k = 0;
    % somme des valeurs propres de l'itération courante
    eigsum = 0.0;
    ...
    while (~conv && k < maxit)
        %On recupère les colonnes qui n'ont pas convergé
        V_nc = Vr(:, nb_c+1:end); 
        k = k+1;
        % Y <- A^p*V
        for i=1:p
        ...
        [Wnc, V_nc] = rayleigh_ritz_projection(A, Vr(:, nb_c+1:end));
        Wr = [Wr(1:nb_c,:); Wnc];
        Vr = [Vr(:,1:nb_c) V_nc];
        
        %% Quels vecteurs ont convergé à cette itération
        analyse_cvg_finie = 0;
        % nombre de vecteurs ayant convergé à l'itération courante
        nbc_k = 0;
